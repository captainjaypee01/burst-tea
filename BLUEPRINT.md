...
- **Destructive confirmations:** Use [`ConfirmDialog`](frontend/src/components/ui/confirm-dialog.tsx) (shadcn **`Dialog`** + explicit Cancel / Confirm) instead of **`window.confirm`**. [`DialogContent`](frontend/src/components/ui/dialog.tsx) supports **`hideClose`** when the corner X should be omitted (confirm flows rely on footer actions only).
- **Scroll utilities:** [`frontend/src/index.css`](frontend/src/index.css) defines **`.scrollbar-thin`** (visible thin bar) and **`.no-scrollbar`** (scrollable but **hidden** scrollbar — Firefox `scrollbar-width: none`, WebKit zero-width track). Use **`.no-scrollbar`** on long **POS product grids** and **catalog dialogs** where a bar is distracting; keep **`.scrollbar-thin`** where a visible affordance helps (e.g. cart line list).
...
- **Ordering / POS** — **implemented** per [`docs/modules/ordering.md`](docs/modules/ordering.md): **`/pos/new`**, **`/pos/:orderId/compose`**, **`/pos/:orderId/checkout`**; **category tabs**, **product cards** with **inline variants**, **per-line notes**, **order summary** (collapsible on desktop + **Sheet** on mobile), **checkout** for **Cash** (**tendered** + **change** UI; API records **amount due** only) / **Maya / GCash** (record-only); **payment** and **cancel-order** steps use **`ConfirmDialog`** before mutating. **Composer product column** scroll uses **`.no-scrollbar`**; cart line list may use **`.scrollbar-thin`**. **Cash register:** **cash/e-wallet** payments post **`sale`** ledger rows when policy requires; **order detail** links to **register ledger history** when a payment has **`shift_id`**. **UX polish:** POS shell uses bounded-height flex + **native `overflow-y-auto`** where Radix **`ScrollArea`** would collapse in nested flex; see changelog **2026-04-19** and **2026-04-18**.
...
### 2026-04-19 — SPA: ConfirmDialog, payment review dialog, hidden scrollbars

- **Frontend — confirmations:** New [`ConfirmDialog`](frontend/src/components/ui/confirm-dialog.tsx) replaces **`window.confirm`** for destructive / high-stakes actions on [`OrderCheckoutPage`](frontend/src/pages/OrderCheckoutPage.tsx) (cancel order), [`OrdersPage`](frontend/src/pages/OrdersPage.tsx), [`CashRegistersPage`](frontend/src/pages/CashRegistersPage.tsx), [`CategoriesPage`](frontend/src/pages/CategoriesPage.tsx), [`ProductDetailPage`](frontend/src/pages/ProductDetailPage.tsx) (remove product / variant). **[`DialogContent`](frontend/src/components/ui/dialog.tsx)** accepts **`hideClose`** for flows that rely on footer buttons only.
- **Frontend — checkout:** Before **`POST /payments`**, a **`Dialog`** summarizes **order total**, **amount due**, **cash received** / **change** (cash), or **e-wallet** copy (Maya/GCash); **Confirm payment** submits ([`OrderCheckoutPage`](frontend/src/pages/OrderCheckoutPage.tsx)).
- **Frontend — scroll:** [`.no-scrollbar`](frontend/src/index.css) on [`OrderComposerPage`](frontend/src/pages/OrderComposerPage.tsx) **product list** and [`ProductCreateDialog`](frontend/src/components/catalog/ProductCreateDialog.tsx) body for scroll **without** a visible bar; **`.scrollbar-thin`** remains for e.g. [`OrderCartPanel`](frontend/src/components/ordering/OrderCartPanel.tsx) line list where a thin bar is acceptable.
- **Docs:** [`docs/modules/ordering.md`](docs/modules/ordering.md) and [`docs/modules/catalog.md`](docs/modules/catalog.md) aligned; [`docs/prompts/module-delivery-workflow.md`](docs/prompts/module-delivery-workflow.md) POS scroll + checkout checklist updated; [`README.md`](README.md) **What is implemented** POS bullet updated for confirm dialogs, cash tendered/change, and **`.no-scrollbar`** on the product menu.

### 2026-04-18 — Checkout: cash tendered and change

- **Frontend:** [`OrderCheckoutPage`](frontend/src/pages/OrderCheckoutPage.tsx) — for **Cash**, staff enter **cash received**; **change to give** is shown (`tendered − amount due`). **Record cash payment** is enabled only when tendered ≥ due. **`POST /payments`** still sends **`amount_cents`** = **remaining balance** (amount applied to the order), not the tendered amount — see [`docs/modules/ordering.md`](docs/modules/ordering.md) §3 / §4.4.
